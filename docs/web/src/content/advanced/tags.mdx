...
\- if set then column has type(manually set, without struct property type checking), Possible types: text, varchar, json, jsonb, integer, bigint, decimal(scale and precision tags also must be set), float, smallfloat
